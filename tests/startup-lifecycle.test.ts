...
test("deadline degrades without saving anchors or marking the layout current",()=>{const degraded=viewSource.match(/private degradeStartup[\s\S]*?\n\s*private finishPhysicsLayout/)?.[0]??"";assert.ok(degraded);assert.doesNotMatch(degraded,/markLayoutCurrent|savePositions/);assert.match(viewSource,/if\(reason==="converged"\)this\.completeStartup\(metrics\);else this\.degradeStartup\("deadline",metrics\)/)});
