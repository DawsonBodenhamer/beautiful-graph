...
import { migrateAdaptivePanelDefaults, migrateNamedDefaults, migrateResponsivePanels, migrateRevision10Panels, migrateRevision15Glow, migrateRevision19Settings, migrateRevision20Settings, migrateRevision25Settings, migrateRevision29Settings, removeLinkCurveState, resolveLegacyPanelGeometry } from "../src/settings-migration.ts";
...
test("obsolete Link Curve state is removed from active and preset forces",()=>{const settings={forces:{curvature:.5},forcePresets:{legacy:{curvature:-.2}}} as unknown as BeautifulGraphSettings;assert.equal(removeLinkCurveState(settings),true);assert.equal("curvature" in settings.forces,false);assert.equal("curvature" in settings.forcePresets.legacy!,false);assert.equal(removeLinkCurveState(settings),false)});
