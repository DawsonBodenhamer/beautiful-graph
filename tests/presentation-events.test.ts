...
test("retained visual systems remain attached to the isolated renderer",()=>{const view=readFileSync(new URL("../src/graph-view.ts",import.meta.url),"utf8");for(const signature of [/getNodeTexture/,/ensureIcon/,/drawLabelBackground/,/applyFocusFrame/,/applySearch/,/updateFolderLens/,/createGroupsPanel/,/createForcesPanel/,/createDisplayPanel/,/placeLinkSprite/])assert.match(view,signature)});
test("active pointer drag retains focus layers without full-scene blur",()=>{const view=readFileSync(new URL("../src/graph-view.ts",import.meta.url),"utf8");assert.match(view,/scene\.filters=this\.focusProgress>\.01&&!this\.dragged\?\[this\.focusBlur\]:\[\]/)});
