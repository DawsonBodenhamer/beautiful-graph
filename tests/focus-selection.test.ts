...
import { expandedFocusIds, toggledFocusRoots, visibleFocusFamilyIds } from "../src/focus-selection.ts";
...
test("focused camera family excludes hidden nodes and second-hop neighbors",()=>{const adjacency=new Map([["root",new Set(["visible","hidden"])],["visible",new Set(["deep"])]]);assert.deepEqual([...visibleFocusFamilyIds("root",adjacency,new Set(["root","visible","deep"]))].sort(),["root","visible"])});
