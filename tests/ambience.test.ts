...
test("the dark vignette has a static low-opacity grayscale dither",()=>{const styles=readFileSync(new URL("../styles.css",import.meta.url),"utf8");const dither=styles.match(/\.beautiful-ambience-backdrop::after\{[^}]+\}/)?.[0]??"";assert.match(dither,/feTurbulence/);assert.match(dither,/type='saturate' values='0'/);assert.match(dither,/stitchTiles='stitch'/);assert.match(dither,/mix-blend-mode:soft-light/);assert.match(dither,/opacity:\.04/)});
