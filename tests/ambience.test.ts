...
test("the dark vignette has a strong post-filter static grayscale dither",()=>{const styles=readFileSync(new URL("../styles.css",import.meta.url),"utf8"),dither=styles.match(/\.beautiful-graph-view::before\{[^}]+\}/)?.[0]??"";assert.doesNotMatch(styles,/\.beautiful-ambience-backdrop::after/);assert.match(dither,/z-index:1/);assert.match(dither,/pointer-events:none/);assert.match(dither,/feTurbulence/);assert.match(dither,/type='saturate' values='0'/);assert.match(dither,/stitchTiles='stitch'/);assert.match(dither,/mix-blend-mode:soft-light/);assert.match(dither,/opacity:\.14/)});
