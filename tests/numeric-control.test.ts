...
test("control wiring uses only scrub/edit readouts and configured default resets",()=>{const source=readFileSync(new URL("../src/graph-view.ts",import.meta.url),"utf8");assert.match(source,/readout\.ondblclick/);assert.match(source,/event\.key==="Enter"/);assert.match(source,/event\.key==="Escape"/);assert.match(source,/setPointerCapture/);assert.match(source,/reset\.onclick=.*apply\(config\.resetValue,true\)/);assert.doesNotMatch(source,/beautiful-numeric-range/);assert.doesNotMatch(source,/type:"range"/)});
