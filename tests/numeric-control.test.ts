...
test("control wiring includes typed commit/cancel, zero reset, pointer capture, and one history entry per gesture",()=>{const source=readFileSync(new URL("../src/graph-view.ts",import.meta.url),"utf8");assert.match(source,/readout\.ondblclick/);assert.match(source,/event\.key==="Enter"/);assert.match(source,/event\.key==="Escape"/);assert.match(source,/setPointerCapture/);assert.match(source,/reset\.onclick=.*apply\(0,true\)/);assert.match(source,/range\.onpointerdown=\(\)=>this\.pushHistory\(\)/)});
