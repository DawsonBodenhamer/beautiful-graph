...
(function installBeautifulGraphBenchmark(global) {
  ...
  const workloads = {
    ...
    async motion(view, duration) {
      ...
      return{burstCount:completions.length,firstTickMaxMs,completionMaxMs,tickGapMaxMs,substepsMax,tuneTargetsPassed:firstTickMaxMs<=100&&completionMaxMs<=8500&&substepsMax<=4096&&tickGapMaxMs<7000};
    },
  ...
  async function runManualOpen() {
    ...
    for(let run=0;run<RUNS;run+=1){
      ...
      while(!["complete","degraded"].includes(view.getStartupMetrics().phase)&&performance.now()-started<3500)await new Promise(requestAnimationFrame);frameRunning=false;const metrics=view.getStartupMetrics(),stableMs=(metrics.presentationStableAt??performance.now())-started,before=new Map(view.model.nodes.map(node=>[node.id,{x:node.x,y:node.y,family:node.family}])),worker=view.worker;
      ...
      if(worker)await new Promise((resolve,reject)=>{const timeout=setTimeout(()=>{worker.removeEventListener("message",onMessage);reject(new Error("Startup parity Tune exceeded 8.5 seconds."))},8500),onMessage=event=>{if(event.data?.type!=="burstComplete")return;clearTimeout(timeout);worker.removeEventListener("message",onMessage);resolve()};worker.addEventListener("message",onMessage);view.runTuneBurst()});
      await waitFor(()=>view.presentationTargetDelta()<=.15,1000);observer.disconnect();const displacements=[],beforeFamilies=new Map(),afterFamilies=new Map();for(const node of view.model.nodes){const prior=before.get(node.id);if(!prior)continue;displacements.push(Math.hypot(node.x-prior.x,node.y-prior.y)*view.scale);for(const [map,point] of [[beforeFamilies,prior],[afterFamilies,node]]){const total=map.get(point.family)||{x:0,y:0,count:0};total.x+=point.x;total.y+=point.y;total.count++;map.set(point.family,total)}}let familyCentroidMaxPx=0;for(const [family,total] of afterFamilies){const prior=beforeFamilies.get(family);if(!prior)continue;familyCentroidMaxPx=Math.max(familyCentroidMaxPx,Math.hypot(total.x/total.count-prior.x/prior.count,total.y/total.count-prior.y/prior.count)*view.scale)}const tuneP95Px=round(percentile95(displacements)),cameraScaleDrift=metrics.cameraScaleDrift??0;
      const frameGapP95Ms=round(percentile95(frameGaps));runs.push({run:run+1,workload:"manualOpenStartup",stableMs:round(stableMs),longTaskCount:longTasks.length,longTaskMaxMs:round(longTasks.length?Math.max(...longTasks):0),frameGapP95Ms,finalOccupancy:round(finalOccupancy),tuneP95Px,familyCentroidMaxPx:round(familyCentroidMaxPx),metrics,passed:metrics.phase==="complete"&&metrics.convergenceReason==="converged"&&(metrics.residuals?.elapsed??Infinity)<=2500&&longTasks.every(duration=>duration<=50)&&frameGapP95Ms<33&&(metrics.postGapP95Jump??0)<=2&&metrics.workerGenerations===1&&metrics.topologyRebuilds===0&&metrics.firstPaintReady&&metrics.firstNodeCount===metrics.finalNodeCount&&metrics.firstEdgeCount===expectedEdges&&metrics.firstGroupLabelCount===expectedLabels&&finalOccupancy>=.84&&finalOccupancy<=.9&&cameraScaleDrift<.002&&tuneP95Px<2&&familyCentroidMaxPx<4});await sleep(250);
    }
    const report={schema:2,capturedAt:nowIso(),protocol:{runs:RUNS,workerDeadlineMs:2500,longTaskTargetMs:50,tuneP95TargetPx:2,familyCentroidTargetPx:4,cameraScaleDriftTarget:.002},runs};await adapter.write(`${OUTPUT_ROOT}/manual-open.json`,`${JSON.stringify(report,null,2)}\n`);return report;
  }
