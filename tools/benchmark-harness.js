...
(function installBeautifulGraphBenchmark(global) {
  ...
  async function runManualOpen(options = {}) {
    ...
    for(let run=0;run<RUNS;run+=1){
      ...
      observer.disconnect();const cameraScaleDrift=metrics.cameraScaleDrift??0,frameGapP95Ms=round(percentile95(frameGaps)),longTaskTimeline=longTaskEntries.map(entry=>({startMs:round(entry.startTime-started),durationMs:round(entry.duration)})),lifecyclePassed=metrics.phase==="complete"&&metrics.workerTerminal==="sleep"&&frameGapP95Ms<33&&(metrics.postGapP95Jump??0)<=2&&metrics.workerGenerations===1&&metrics.topologyRebuilds===0&&progressivePaint&&initialOccupancy>=.84&&initialOccupancy<=.9&&cameraScaleDrift<.002,strictPassed=lifecyclePassed&&longTasks.every(duration=>duration<=50);runs.push({run:run+1,workload:"manualOpenStartup",stableMs:round(stableMs),longTaskCount:longTasks.length,longTaskMaxMs:round(longTasks.length?Math.max(...longTasks):0),longTaskTimeline,frameGapP95Ms,initialOccupancy:round(initialOccupancy),progressivePaint,metrics,lifecyclePassed,strictPassed,passed:lifecyclePassed});await sleep(250);
    }
    const finalView=app.workspace.getLeavesOfType("beautiful-graph").at(-1)?.view;
    const report={schema:3,capturedAt:nowIso(),...(await releaseMetadata(plugin,finalView,options.commit)),protocol:{runs:RUNS,startupTerminal:"worker-sleep",longTaskTargetMs:50,startupLongTaskPolicy:"record-only-user-approved",initialOccupancyRange:[.84,.9],maxNodesPerFrame:50,cameraScaleDriftTarget:.002},runs};await adapter.write(`${OUTPUT_ROOT}/manual-open.json`,`${JSON.stringify(report,null,2)}\n`);return report;
  }
