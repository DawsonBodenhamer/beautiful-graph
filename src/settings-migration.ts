...
export function removeLinkCurveState(settings:BeautifulGraphSettings):boolean {let changed=false;const strip=(force:BeautifulGraphSettings["forces"]|undefined)=>{if(force&&"curvature" in force){delete (force as BeautifulGraphSettings["forces"]&{curvature?:unknown}).curvature;changed=true}};strip(settings.forces);for(const preset of Object.values(settings.forcePresets??{}))strip(preset);return changed}
