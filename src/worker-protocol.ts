...
export const NORMAL_COOLING_TICKS=300;
export const STARTUP_COOLING_TICKS=780;
export const TOPOLOGY_COOLING_TICKS=600;
export const TOPOLOGY_WARMUP_TICKS=180;
export const alphaDecayForTicks=(ticks:number)=>1-Math.pow(ALPHA_MIN,1/ticks);
export const ALPHA_DECAY=alphaDecayForTicks(NORMAL_COOLING_TICKS);
...
export type WorkerInitMessage={type:"init";version:typeof WORKER_PROTOCOL_VERSION;revision:number;nodes:WorkerNode[];edges:GraphEdge[];forces:GraphForces;heat?:number;coolingTicks?:number};
export type WorkerTopologyMessage={type:"topology";version:typeof WORKER_PROTOCOL_VERSION;revision:number;nodes:WorkerNode[];edges:GraphEdge[];heat?:number;coolingTicks?:number;warmupTicks?:number};
