...
export function buildGraphModel(app: App, settings: BeautifulGraphSettings): GraphModel {
  ...
  const nodes: GraphNode[] = files.map((file) => {
    ...
    const metadata=typeof frontmatter==="object"&&frontmatter!==null?frontmatter as Record<string,unknown>:undefined;
    ...
    return {
      ...
      category, color: group?.color??settings.other.color, icon:stringMetadata(metadata?.graph_icon)??group?.icon??settings.other.icon??icon, degree: d,
      baseRadius:radius, radius, description:stringMetadata(metadata?.description)??"", visible: nodeAllowed(file.path,settings.groups,settings.other.visible)&&orphanAllowed(d,settings.display.showOrphans), hub,
