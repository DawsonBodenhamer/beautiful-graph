...
export class BeautifulGraphSettingTab extends PluginSettingTab {
  ...
  display():void {
    ...
    new Setting(this.containerEl)
      ...
      .addSlider(slider=>slider.setLimits(0,15,1).setDynamicTooltip().setValue(this.plugin.settings.storedNodeCount).onChange(async value=>{this.plugin.settings.storedNodeCount=value;await this.plugin.persistData()}));
    new Setting(this.containerEl)
      ...
      .addSlider(slider=>slider.setLimits(.5,2,.01).setDynamicTooltip().setValue(this.plugin.settings.display.recenterMultiplier).onChange(async value=>{this.plugin.settings.display.recenterMultiplier=value;await this.plugin.persistData()}));
