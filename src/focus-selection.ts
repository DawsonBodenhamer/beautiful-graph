...
export function visibleFocusFamilyIds(rootId:string,adjacency:ReadonlyMap<string,ReadonlySet<string>>,visibleIds:ReadonlySet<string>):Set<string> {
  const family=expandedFocusIds(new Set([rootId]),adjacency);return new Set([...family].filter(id=>visibleIds.has(id)));
}
